...
OpenBank = function(data) {
    ...
    $('.firstname').html('<p>Добре дошли, <b>'+ data.chardata.charinfo.firstname + ' ' + data.chardata.charinfo.lastname +'</b>.</p>')
    $('#private').data('saldo', data.chardata.money['bank'])
    $('#private').data('bankid', data.chardata.charinfo.account)
    $('#private').html('<div class="account-card-container"><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+ data.chardata.charinfo.firstname + ' ' + data.chardata.charinfo.lastname +'</p><p id="account-number" class="account-number">' + data.chardata.charinfo.account + '</p><p id="account-amount" class="account-amount">Баланс: €' + data.chardata.money['bank'] + '</p></div></div>');
    setTimeout(function() {
    $.post('http://turbo-banking/GetPrivateAcounts', JSON.stringify({}), function(Accounts){
        for (const [key, value] of Object.entries(Accounts)) {
         Count = parseInt(Count) + parseInt(key)
         var OpenAccounts = '<div class="delete-shared-account" data-bankid='+value.BankId+' style="float:left;margin-top:25px; position: absolute; left: 2px;"><i class="fas fa-times" style="font-size: 15px; color: red;"></i></div><div class="account-card" id="slot-'+key+'" data-saldo="'+value.Balance+'" data-bankid ="'+value.BankId+'" data-owner="'+value.Owner+'" data-slot="'+key+'"><div class="account-card-container" ><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+ value.Name +'</p><p id="account-number" class="account-number">'+ value.BankId +'</p><p id="account-amount" class="account-amount">Баланс: €'+ value.Balance +'</p></div></div>';
    ...
    $.post('http://turbo-banking/GetSharedAccounts', JSON.stringify({}), function(Accounts){
        for (const [key, value] of Object.entries(Accounts)) {
...
RefreshBank = function() {
    ...
    $('.personal-accounts').html('')
    $('.shared-accounts').html('')
    $.post('http://turbo-banking/GetPersonalBalance', JSON.stringify({}), function(Data){
        $('#private').data('saldo', Data.Balance)
        $('#private').data('bankid', Data.BankId)
        $('#private').html('<div class="account-card-container"><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+ Data.Name +'</p><p id="account-number" class="account-number">' + Data.BankId + '</p><p id="account-amount" class="account-amount">Баланс: €' + Data.Balance + '</p></div></div>');
    $.post('http://turbo-banking/GetPrivateAcounts', JSON.stringify({}), function(Accounts){
        for (const [key, value] of Object.entries(Accounts)) {
         Count = parseInt(Count) + parseInt(key)
         var OpenAccounts = '<div class="delete-shared-account" data-bankid='+value.BankId+' style="float:left;margin-top:25px; position: absolute; left: 2px;"><i class="fas fa-times" style="font-size: 15px; color: red;"></i></div><div class="account-card" id="slot-'+key+'" data-saldo="'+value.Balance+'" data-bankid ="'+value.BankId+'" data-owner="'+value.Owner+'" data-slot="'+key+'"><div class="account-card-container" ><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+ value.Name +'</p><p id="account-number" class="account-number">'+ value.BankId +'</p><p id="account-amount" class="account-amount">Баланс: €'+ value.Balance +'</p></div></div>';
...
OnClick = function(type) {
  $.post('http://turbo-banking/ClickSound', JSON.stringify({
      success: type
  }))
}
...
CloseBankApp = function() {
    ...
    $.post('http://turbo-banking/CloseApp', JSON.stringify({}))
}

SetupTransactionForAccount = function(BankNumber) {
  $.post('http://turbo-banking/GetTransactions', JSON.stringify({
      BankId: BankNumber,
  }));
}
...
$(document).on('click', '.account-card', function(e) {
    ...
    if ($(this).data('bankid') != '') {
        ...
        $('.bankid').html('<p>Банкова сметка: '+ ClickedData.bankid +'</p>')
        $('.bank-main-amount').html('<p>Наличен баланс</p>€'+ ClickedData.balance +'')
        ...
        if ($(this).data('slot') >= 2) {
          $.post('http://turbo-banking/GetAccountUsers', JSON.stringify({
...
$(document).on('click', '#create-account', function(e) {
    ...
    if (BankName !== undefined && BankName !== '') {
        ...
        if (SelectedSlot >= 2) {
            $(".card-create-input").val('');
            $(".bank-card-create-holder").fadeOut(750);
            $.post('http://turbo-banking/CreateAccount', JSON.stringify({
              Name: BankName,
              Slot: 2,
              Type: 'shared',
            }))
...
$(document).on('click', '#withdraw', function(e) {
    ...
    if (MoneyAmount > 0 && SelectedBank !== null) {
     $.post('http://turbo-banking/Withdraw', JSON.stringify({
         BankId: SelectedBankData.bankid,
...
$(document).on('click', '#deposit', function(e) {
    ...
    var MoneyAmount = parseInt($(".bank-form").val());
    if (MoneyAmount > 0 && SelectedBank !== null) {
     $.post('http://turbo-banking/Deposit', JSON.stringify({
         BankId: SelectedBankData.bankid,
...
$(document).on('click', '.delete-shared-account', function(e) {
    ...
    if (BankId != null && BankId != undefined) {
        $.post('http://turbo-banking/DeleteAccount', JSON.stringify({
            BankId: BankId,
        }))
        setTimeout(function() {
            RefreshBank()
...
$(document).on('click', '#submit-add', function(e) {
    ...
    if (BsnNumber.length == 8) {
        OnClick('success-click2')
        $.post('http://turbo-banking/AddUserToAccount', JSON.stringify({
            BankId: SelectedBankData.bankid,
            TargetBsn: BsnNumber
        }))
        $(".add-account-bsn").val('');
...
$(document).on('click', '.bank-user-delete', function(e) {
    ...
    if (ClickedCitizenId !== undefined && ClickedCitizenId !== '') {
        if (ClickedCitizenId !== SelectedBankData.owner) {
         $.post('http://turbo-banking/DeleteFromAccount', JSON.stringify({

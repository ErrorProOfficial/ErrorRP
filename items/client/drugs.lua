...
RegisterNetEvent('items:client:use:joint')
AddEventHandler('items:client:use:joint', function()
  RePCore.Functions.TriggerCallback('RePCore:HasItem', function(HasItem)
		if HasItem then
      Citizen.SetTimeout(1000, function()
        TriggerServerEvent('RePCore:Server:RemoveItem', 'joint', 1)
        TriggerEvent('animations:client:EmoteCommandStart', {"smoke"})
        TriggerEvent('inventory:client:set:busy', true)
        local ped = PlayerPedId();
        RePCore.Functions.Progressbar("smoke-joint", "Запалване на джойнта..", 9700, false, true, {
        ...
        disableCombat = true,
          ...
          TriggerEvent('animations:client:EmoteCommandStart', {"c"})
            ...
            TriggerEvent("inventory:client:ItemBox", RePCore.Shared.Items["joint"], "remove")
            TriggerEvent('items:client:joint:effect')
            TriggerEvent("police:client:SetStatus", "weedsmell", 200)
          end, function() -- Cancel
            TriggerEvent('inventory:client:set:busy', false)
...
RegisterNetEvent("items:client:use:coke")
AddEventHandler("items:client:use:coke", function()
   Citizen.SetTimeout(1000, function()
    TriggerServerEvent("RePCore:Server:RemoveItem", "coke-bag", 1)
    TriggerEvent("inventory:client:ItemBox", RePCore.Shared.Items["coke-bag"], "remove")
     TriggerEvent('inventory:client:set:busy', true)
     RePCore.Functions.Progressbar("snort_coke", "Взимане на кокаин..", math.random(5000, 8000), false, true, {
         disableMovement = false,
         disableCarMovement = false,
         disableMouse = false,
         disableCombat = true,
         ...
         TriggerEvent("police:client:SetStatus", "widepupils", 200)
         CokeBagEffect()
     end, function() -- Cancel
         TriggerEvent('inventory:client:set:busy', false)
         StopAnimTask(PlayerPedId(), "anim@amb@nightclub@peds@", "missfbi3_party_snort_coke_b_male3", 1.0)
...
RegisterNetEvent("items:client:use:lsd")
AddEventHandler("items:client:use:lsd", function()
  Citizen.SetTimeout(1000, function()
    TriggerEvent('inventory:client:set:busy', true)
    TriggerServerEvent("RePCore:Server:RemoveItem", "lsd-strip", 1)
    RePCore.Functions.Progressbar("snort_coke", "Взимане на таблетка..", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
       ...
       flags = 49,
        ...
        TriggerEvent("inventory:client:ItemBox", RePCore.Shared.Items["lsd-strip"], "remove")
        TriggerEvent("police:client:SetStatus", "widepupils", 200)
        Effectlsd()
    end, function() -- Cancel
        TriggerEvent('inventory:client:set:busy', false)
...
RegisterNetEvent('items:client:joint:effect')
AddEventHandler('items:client:joint:effect', function()
    OnWeed = true
    weedEffect()
    while OnWeed do
        Citizen.Wait(0)
        if WeedTime > 0 then
            WeedTime = WeedTime - 1
            TriggerServerEvent('hud:server:remove:stress', math.random(4, 10))
            Citizen.Wait(1000)
        end
        if WeedTime == 0 then
            OnWeed = false
...
RegisterNetEvent('items:client:painkiller:effect')
AddEventHandler('items:client:painkiller:effect', function()
            TriggerServerEvent('hud:server:remove:stress', math.random(5, 10))
end)

...
local FireworkList = {
    ...
    ["scr_indep_fireworks"] = {
        ...
        "scr_indep_firework_fountain",	-- addon
    },
    ["proj_indep_firework"] = {
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if fireworkTime > 0 and fireworkLoc ~= nil then
            DrawText3Ds(fireworkLoc.x, fireworkLoc.y, fireworkLoc.z, "Фоерверки на ~r~"..fireworkTime)
        end
    end
end)
...
AddEventHandler("fireworks:client:UseFirework", function(itemName, assetName)
    TriggerServerEvent("TurboCore:Server:RemoveItem", itemName, 1)
    TurboCore.Functions.Progressbar("spawn_object", "Поставяме фоерверк..", 3000, false, true, {
        ...
        TriggerEvent("inventory:client:ItemBox", TurboCore.Shared.Items[itemName], "remove")
        local pos = GetEntityCoords(PlayerPedId())
        DoFireWork(assetName, pos)
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        TurboCore.Functions.Notify("Отказано..", "error")
    end)
end)
...
function DoFireWork(asset, coords)
    ...
    Citizen.CreateThread(function()
        ...
        for i = 1, math.random(5, 10), 1 do
            ...
            local part = StartNetworkedParticleFxNonLoopedAtCoord(firework, fireworkLoc.x + 30.0, fireworkLoc.y + 30, fireworkLoc.z + 42.5, 0.0, 0.0, 0.0, math.random() * 0.3 + 0.5, false, false, false, false)

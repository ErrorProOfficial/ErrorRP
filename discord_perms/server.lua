...
-- discord queue turbo
function GetRolesQueue(user)
	local discordId = nil
	for _, id in ipairs(user) do
		if string.match(id, "discord:") then
			discordId = string.gsub(id, "discord:", "")
		--TURBOOOO	print("Found discord id: "..discordId)
			break
		end
	end
	...
	if discordId then
		...
		--TURBOOOO	print("An error occured, maybe they arent in the discord? Error: "..member.data)
			return false
		end
	else
	--	print("missing identifier B")	-- was spamming in the console 
		return false
	end
end
-- discord queue turbo
...
exports("GetRolesQueue", GetRolesQueue)